...
test("entity-refs: ref()+deref() hot loop (backrefs OFF)", async ({
  bench,
}) => {
  await bench.compare(
...
test("entity-refs: ref(target,holder) registration (backrefs ON)", async ({
  bench,
}) => {
  await bench.compare(
...
test("entity-refs: flush() sweep cost", async ({ bench }) => {
  await bench.compare(
