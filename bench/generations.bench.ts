...
test("generations: handleOf + resolve round-trip", async ({ bench }) => {
  ...
  await bench("handleOf + resolve round-trip (1000 live entities)", () => {
    let sum = 0;
...
test("generations: flush() despawn cost is unchanged by the generation bump", async ({
  bench,
}) => {
  await bench.compare(
